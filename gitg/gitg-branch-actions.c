              ...
              ...)
{
	...
	GitgCommand *command = gitg_command_new (argv);
	g_free (argv);
...
get_stash_refspec (GitgRepository *repository, GitgRef *stash)
{
	...
	GitgCommand *command = gitg_command_newv ("log",
	                                            "--no-color",
	                                            "--pretty=oneline",
	                                            "-g",
	                                            "refs/stash",
	                                            NULL);
	out = gitg_repository_command_with_output (repository, command, NULL);
	g_object_unref (command);
...
stash_changes_real (GitgWindow *window, gchar **ref, gboolean storeref)
{
	...
	GitgCommand *command = gitg_command_newv ("commit-tree", tree, "-p", head, NULL);
	gitg_repository_run_command_with_input (repository, runner, command, idxmsg, NULL);
	...
	gitg_command_add_environment (command, "GIT_INDEX_FILE", tmpname);
	g_free (tmpname);
	
	gboolean writestash = TRUE;
	
	if (writestash == TRUE)
	{
		gitg_command_set_argumentsv (command, "read-tree", "-m", tree, NULL);
		writestash = gitg_repository_run_command (repository, runner, command, NULL);
	}
	if (writestash == TRUE)
	{
		gitg_command_set_argumentsv (command, "add", "-u", NULL);
		writestash = gitg_repository_run_command (repository, runner, command, NULL);
	}
	if (writestash == TRUE)
	{
		gitg_command_set_argumentsv (command, "write-tree", NULL);
		writestash = gitg_repository_run_command (repository, runner, command, NULL);
	}
	...
	gitg_command_set_environment (command, NULL);
	...
	gitg_command_set_argumentsv (command, "commit-tree", stashtree,
	                             "-p", head,
	                             "-p", commit, NULL);
	gitg_repository_run_command_with_input (repository, runner, command, reason, NULL);
	g_object_unref (command);
